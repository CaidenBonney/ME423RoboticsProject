...
# ==========================
# ADDED: homography interface imports (function calls)
# ==========================
...
try:
    ...
    # ==========================
    # ADDED: load calibration once for ArUco/homography detection
    # ==========================
    ...
    # ==========================
    # ADDED: one-time homography calibration (after background subtractor warmup)
    # ==========================
    homography = None
    origin_uv = None
    axis_p1 = None
    ...
    while homography is None:
        ...
        frame_bgr = np.asanyarray(rgb_frames.get_data())
    ...
    while True:
        ...
        if rgb_frames and depth_frames:
            ...
            if best is not None:
                ...
                # ==========================
                # ADDED: ball center -> world XY + distance from origin ArUco 67
                # ==========================
                try:
                    x_m, y_m = pixel_source_to_world_xy_m(
                        uv_source=(cx, cy),
                        H=homography,
                    ...
                    dist_m = math.sqrt(x_m * x_m + y_m * y_m)

                    cv2.putText(out, f"XY(m): x={x_m:.3f}, y={y_m:.3f}", (10, 90),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                    cv2.putText(out, f"Dist from origin (m): {dist_m:.3f}", (10, 120),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                except Exception:
                    cv2.putText(out, "XY(m): map failed", (10, 90),
