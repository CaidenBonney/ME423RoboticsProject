...
cap = cv2.VideoCapture("src\\videos\\rgb_video_arc.mp4") # This number may be different for every machine. It corresponds to the port that the camera is attached to
if not cap.isOpened():
...
while True:
    ...
    edges = cv2.Canny(blurred, 50, 150) # The output is a binary image (edge map)
