...
class KalmanTrajectory:
    ...
    # -----------------------------
    def update_trajectory(self, measurement_xyz, timestamp: float) -> np.ndarray:
        """
    ...
    # Forward prediction
    # -----------------------------
    def predict_state_dt(self, dt: float) -> Optional[np.ndarray]:
        """
    ...
    def predict_position(self, timestamp: float) -> Optional[np.ndarray]:
        dt = timestamp - self.last_timestamp
        pred = self.predict_state_dt(dt, state=self.kf.current_state)
        return None if pred is None else pred[:3]

    # -----------------------------
    # Plane intersection
    # -----------------------------
    def estimateCatchTime(
        self,
        z_plane: float,
...
# state = traj.update_trajectory(measurement_xyz, timestamp)
