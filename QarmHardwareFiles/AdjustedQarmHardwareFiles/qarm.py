...
class QArm():
    ...
    def _set_double_properties_individually(self, properties, values):
        for prop, value in zip(properties, values):
            self.card.set_double_property(
                np.array([prop], dtype=np.int32),
                1,
    ...
    def __init__(self, hardware=1, readMode=0, frequency=500, deviceId = 0, hilPortNumber = 18900):
        ...
        boardSpecificOptions = (
            f"j0_mode={int(self.mode[0])};j1_mode={int(self.mode[1])};"
            f"j2_mode={int(self.mode[2])};j3_mode={int(self.mode[3])};"
            f"gripper_mode={int(self.mode[4])};"
            f"j0_profile_config=0;j0_profile_velocity={self.profileVelocity[0]};j0_profile_acceleration={self.profileAcceleration[0]};"
            f"j1_profile_config=0;j1_profile_velocity={self.profileVelocity[1]};j1_profile_acceleration={self.profileAcceleration[1]};"
            ...
            if self.card.is_valid():
                ...
                if self.readMode == 1:
                    ...
                    if self.hardware:
                        ...
                        self._set_double_properties_individually(pidProperties, pidValues)
