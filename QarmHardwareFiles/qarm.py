...
class QArm:
    ...
    WRITE_OTHER_CHANNELS = np.array(
        [1000, 1001, 1002, 1003, 1004, 11000, 11001, 11002, 11003, 11005, 11006, 11007],
        dtype=np.int32,
    )
    ...
    def __init__(
        self, hardware=1, readMode=0, frequency=500, deviceId=0, hilPortNumber=18900
    ):
        ...
        if self.hardware:
            ...
            boardIdentifier = (
                "0@tcpip://localhost:" + str(hilPortNumber) + "?nagle='off'"
            )
        ...
        try:
            ...
            if self.card.is_valid():
                self.card.set_card_specific_options(
                    boardSpecificOptions, MAX_STRING_LENGTH
                )
                self.status = True
                if self.readMode == 1:
                    ...
                    if self.hardware:
                        self.card.task_set_buffer_overflow_mode(
                            self.readTask, BufferOverflowMode.OVERWRITE_ON_OVERFLOW
                        )
                        ...
                        self.card.task_set_buffer_overflow_mode(
                            self.readTask, BufferOverflowMode.WAIT_ON_OVERFLOW
                        )

                    # Start the reading task
                    self.card.task_start(
                        self.readTask, Clock.HARDWARE_CLOCK_0, self.frequency, 2**32 - 1
                    )
    ...
    def read_write_std(
        ...
        """
        self.writeOtherBuffer[4] = np.clip(
            grpCMD, 0.1, 0.9
        )  # Saturate gripper between 0.1 (open) and 0.9 (close)
